...
const ListPesanan = ({ navigation, route }) => {
  // Konteks untuk menyimpan list pesanan agar konsisten
  let { listPesanan, setListPesanan } = useContext(PesananContext)
  // Ambil dan tambahkan detail pemesanan dari rute sebelumnya (bila ada)
  let { tambahkan, detailPesanan } = route.params
  if (tambahkan) {
    if (!listPesanan.includes(detailPesanan)) {
      listPesanan.push(detailPesanan)
    }
    tambahkan = false
  }
        ...
        <View style={styles.modalCenter}>
          <View style={styles.modalPopup}>
            ...
            <View style={{paddingBottom: 8}}>
              ...
              <Text><Bold>Harga:</Bold> Rp {tiketBatal.totalHarga}</Text>
            </View>
            <Text>Tekan "Lanjutkan" bila Anda yakin!</Text>
            <Text>Aksi ini tidak dapat dibatalkan.</Text>
            <View style={{marginTop: 12, flexDirection: 'row', justifyContent: 'flex-end'}}>
              ...
              <TouchableOpacity onPress={() => {
                ...
                // Ganti status tiket yang dibatalkan
                listPesanan.forEach((item, i) => {
                  if (item == tiketBatal) listPesanan[i].status = "Dibatalkan"
                })
                setListPesanan(listPesanan)
                // Alihkan ke tab pembatalan
                navigation.navigate("Pembatalan", {
                  screen: "Riwayat Pembatalan",
                  params: { listPesanan: listPesanan }
                })
              }}>
                <Text style={[styles.modalButton, {backgroundColor: '#D32F2F'}]}>Ya, Batalkan</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
  ...
  const filterPesanan = listPesanan.filter(pesanan => pesanan.status == "Dipesan")
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={[styles.card, {alignItems: "center", justifyContent: "center", padding: 20, marginTop: 24}]}>
              <Ionicons name="warning" size={96} color="#FFA000"/>
              ...
              <TouchableOpacity style={[styles.card, {marginTop: (item == filterPesanan[0]) ? 24 : 0} ]} onPress={() => {setTiketBatal(item); setTampilModal(true)}}>
                ...
                <View style={styles.cardSection}>
                  <View>
                    ...
                    <Ionicons name="checkmark-circle" size={36} style={{marginHorizontal: 6}} color="#4CAF50"/>
                  </View>
                  <View>
                    <Text><Bold>Info Lainnya:</Bold></Text>
                    <Text>Status: Terverifikasi</Text>
