...
import DataJadwal from '../misc/data.json'

const PilihJadwal = ({ navigation, route }) => {
  // Simpan state/variabel dari rute/halaman sebelumnya
  const { asal, tujuan, kelas, tanggal, jam } = route.params
  const [ tampilModal, setTampilModal ] = useState(false)
  const [ idKapal, setIdKapal ] = useState(null)

  // Filter data jadwal agar sesuai kriteria pencarian
  const filterJadwal = DataJadwal.filter(jadwal => {
      ...
      >
        <View style={styles.modalCenter}>
          <View style={styles.modalPopup}>
            ...
            <Text><Bold>Kelas Layanan:</Bold> {kelas}</Text>
            ...
            <View style={{marginTop: 12, flexDirection: 'row', justifyContent: 'flex-end'}}>
              ...
              <TouchableOpacity onPress={() => {
                navigation.navigate("Pesan Tiket", {
                  idKapal: idKapal,
                  asal: asal,
                  tujuan: tujuan,
                  kelas: kelas,
                ...
                <Text style={styles.modalButton}>Lanjutkan</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
    ...
    <View style={styles.container}>
      <View style={styles.body}>
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            return (
              <TouchableOpacity style={[styles.card, {marginTop: (item.index == filterJadwal[0].index) ? 24 : 0} ]} onPress={() => {setIdKapal(item.id_kapal); setTampilModal(true)}}>
